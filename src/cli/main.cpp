// Module: Snow CLI command parsing and command dispatch.
// Why: keep command parsing stable while mapping legacy emit flags onto structured dump options.
...
namespace {
    ...
    bool ParseDumpSpec(const std::string &spec, snow::driver::EmitOptions &emit, std::string &error) {
        if (spec.empty()) {
            error = "empty dump spec";
            return false;
        }
        ...
        while (std::getline(iss, token, ',')) {
            if (token == "tokens") {
                ...
                emit.llvm = true;
            } else if (token == "timings") {
                emit.timings = true;
            } else if (!token.empty()) {
                error = "unknown dump item: " + token;
                return false;
            }
        }
    ...
    bool ParseCommonOptions(const std::vector<std::string> &args, ParsedOptions &parsed, std::string &error) {
        for (std::size_t i = 0; i < args.size(); ++i) {
            ...
            if (arg == "--dump") {
                if (i + 1 >= args.size()) {
                    error = "missing dump spec after --dump";
                    return false;
                }
                if (!ParseDumpSpec(args[++i], parsed.emit, error)) {
                    return false;
                }
                continue;
            }
            ...
            if (arg.rfind("--dump=", 0) == 0) {
                if (!ParseDumpSpec(arg.substr(std::string("--dump=").size()), parsed.emit, error)) {
                    return false;
                }
                continue;
            }
    ...
    void PrintCompileDumps(const snow::driver::CompileResult &result, const snow::driver::CompileRequest &request) {
        ...
        if (!result.timing_dump.empty()) {
            std::cout << result.timing_dump;
        }
    }
    ...
    int RunCompile(const std::vector<std::string> &args) {
        ...
        request.write_artifact = parsed.write_artifact;
