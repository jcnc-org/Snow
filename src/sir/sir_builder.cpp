// Module: SIR construction from sema + ownership facts.
// Invariant: emitted SIR preserves sema typing/visibility decisions and explicit CFG shape.
...
namespace snow::sir {
            ...
            switch (expr->kind) {
                ...
                case snow::frontend::Expr::Kind::Call: {
                    ...
                    for (const auto &arg: expr->args) {
                        const EmittedValue arg_value = EmitExpr(arg, symbol_types, symbol_ptrs, callee_symbols,
                                                                callee_return_types, instructions, next_ssa_id);
                ...
                case snow::frontend::Expr::Kind::Binary: {
                    ...
                    const EmittedValue lhs = EmitExpr(expr->lhs, symbol_types, symbol_ptrs, callee_symbols,
                                                      callee_return_types, instructions, next_ssa_id);
                    const EmittedValue rhs = EmitExpr(expr->rhs, symbol_types, symbol_ptrs, callee_symbols,
                                                      callee_return_types, instructions, next_ssa_id);
    ...
    Module SirBuilder::Build(const snow::sema::SemaModule &sema_module,
        ...
        for (const auto &function_ast: sema_module.ast.functions) {
            ...
            const std::string mangled = snow::common::MangleSymbol(sema_module.ast.module_path, function_ast.name,
                                                                   param_types, function_ast.return_type, false);
            callee_symbols[function_ast.name] = mangled;
            callee_return_types[function_ast.name] =
                    function_ast.return_type.empty() ? "i32" : function_ast.return_type;
        }
        ...
        for (const auto &function_ast: sema_module.ast.functions) {
            ...
            function.name = callee_it != callee_symbols.end()
                                    ? callee_it->second
                                    : snow::common::MangleSymbol(sema_module.ast.module_path, function_ast.name,
                                                                 param_types, function_ast.return_type, false);
            ...
            auto block_terminated = [&](const std::size_t index) { return BlockTerminated(function.blocks[index]); };
                ...
                for (const auto &stmt: statements) {
                    ...
                    switch (stmt.kind) {
                        ...
                        case snow::frontend::Statement::Kind::Assign: {
                            ...
                            if (stmt.expr) {
                                assigned = EmitExpr(stmt.expr, symbol_types, symbol_ptrs, callee_symbols,
                                                    callee_return_types,
                                                    function.blocks[current_block_idx].instructions, next_ssa_id);
                            }
                            function.blocks[current_block_idx].instructions.push_back(Instruction{
                        ...
                        case snow::frontend::Statement::Kind::Let: {
                            ...
                            if (stmt.expr) {
                                init = EmitExpr(stmt.expr, symbol_types, symbol_ptrs, callee_symbols,
                                                callee_return_types, function.blocks[current_block_idx].instructions,
                                                next_ssa_id);
                            }
                            if (!stmt.name.empty()) {
                                const std::string value_type = !stmt.type_name.empty()
                                                                       ? stmt.type_name
                                                                       : (init.type.empty() ? "i32" : init.type);
                                const std::string slot_name = "%" + std::to_string(next_ssa_id++);
                                function.blocks[current_block_idx].instructions.push_back(Instruction{
                                        .result = slot_name,
                                        .type = "ptr",
                        ...
                        case snow::frontend::Statement::Kind::If: {
                            ...
                            function.blocks[current_block_idx].instructions.push_back(Instruction{
                                    ...
                                    .operands = {cond_value.value, function.blocks[then_idx].label,
                                                 function.blocks[else_idx].label},
                                    .is_terminator = true,
                            });
                        ...
                        case snow::frontend::Statement::Kind::While: {
                            ...
                            function.blocks[current_block_idx].instructions.push_back(Instruction{
                                    ...
                                    .type = "void",
                                    .opcode = Opcode::CondBr,
                                    .operands = {cond_value.value, function.blocks[body_idx].label,
                                                 function.blocks[exit_idx].label},
                                    .is_terminator = true,
                            });
