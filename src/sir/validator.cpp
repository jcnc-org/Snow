// Module: SIR structural/type/lifetime validation.
// Invariant: validation is read-only and emits deterministic diagnostics for equivalent input IR.
...
namespace snow::sir {
    ...
    namespace {
        ...
        bool LooksLikeValue(const std::string &operand) { return !operand.empty() && operand[0] == '%'; }
        ...
        bool IsPointerType(const std::string &type) { return type == "ptr" || type.rfind("ptr<", 0) == 0; }
    ...
    ValidationReport SirValidator::Validate(const Module &module, const ValidationLevel level,
        ...
        const std::string diag_file = module.source_path.empty() ? module.module_path : module.source_path;
        ...
        for (const auto &function: module.functions) {
            const snow::common::SourceRange function_diag_range = NormalizeRange(function.range);
            ...
            for (const auto &block: function.blocks) {
                const snow::common::SourceRange block_diag_range =
                        block.instructions.empty() ? function_diag_range
                                                   : NormalizeRange(block.instructions.front().range);
                if (block.instructions.empty()) {
                    diagnostics.Error("E_SIR_EMPTY_BLOCK", "BasicBlock has no instructions: " + block.label, diag_file,
                                      block_diag_range);
                    report.ok = false;
                    continue;
                }
                ...
                for (std::size_t i = 0; i < block.instructions.size(); ++i) {
                    ...
                    const snow::common::SourceRange current_range =
                            HasRange(instr.range) ? instr.range : function_diag_range;

                    if (instr.opcode == Opcode::Phi && non_phi_seen) {
                        diagnostics.Error("E_SIR_PHI_ORDER", "phi instruction must be at block start", diag_file,
                                          current_range);
                        report.ok = false;
                    }
                    ...
                    if (instr.result.has_value()) {
                        if (defs.contains(instr.result.value())) {
                            diagnostics.Error("E_SIR_SSA_REDEF", "SSA redefinition: " + instr.result.value(), diag_file,
                                              current_range);
                            report.ok = false;
                        }
                    ...
                    for (const auto &operand: instr.operands) {
                        if (LooksLikeValue(operand) && !defs.contains(operand) && instr.opcode != Opcode::Phi) {
                            diagnostics.Warning("W_SIR_USE_BEFORE_DEF", "Use before def candidate: " + operand,
                                                diag_file, current_range);
                        }
                    }
                    ...
                    if (IsTerminator(instr.opcode) || instr.is_terminator) {
                        ...
                        if (i + 1 != block.instructions.size()) {
                            diagnostics.Error("E_SIR_TERM_NOT_LAST", "Terminator must be final instruction", diag_file,
                                              current_range);
                            report.ok = false;
                        }
                    }
                    ...
                    switch (instr.opcode) {
                        ...
                        case Opcode::Ge:
                            if (instr.operands.size() != 2) {
                                diagnostics.Error("E_SIR_ARITY", "Binary instruction requires 2 operands", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else {
                                ...
                                if (lhs_type.has_value() && rhs_type.has_value() &&
                                    ...
                                    diagnostics.Error("E_SIR_TYPE_MISMATCH", "Binary operands have incompatible types",
                                                      diag_file, current_range);
                                    report.ok = false;
                                }
                                if (instr.opcode == Opcode::Add || instr.opcode == Opcode::Sub ||
                                    ...
                                    if (!instr.type.empty() && !IsNumericType(instr.type)) {
                                        diagnostics.Error("E_SIR_TYPE_ARITH",
                                                          "Arithmetic instruction result must be numeric", diag_file,
                                                          current_range);
                                        report.ok = false;
                                    }
                        ...
                        case Opcode::CondBr:
                            if (instr.operands.size() != 3) {
                                diagnostics.Error("E_SIR_CONDBR_ARITY", "cond_br requires condition and two targets",
                                                  diag_file, current_range);
                                report.ok = false;
                            } else {
                                const auto cond_type = InferOperandType(instr.operands[0], value_types);
                                if (cond_type.has_value() && !IsBooleanType(cond_type.value())) {
                                    diagnostics.Error("E_SIR_CONDBR_TYPE", "cond_br condition must be bool/i1",
                                                      diag_file, current_range);
                                    report.ok = false;
                                }
                                ...
                                if (!block_labels.contains(true_label) || !block_labels.contains(false_label)) {
                                    diagnostics.Error("E_SIR_CFG_TARGET", "cond_br target label does not exist",
                                                      diag_file, current_range);
                                    report.ok = false;
                                }
                            }
                        ...
                        case Opcode::Br:
                            if (instr.operands.size() != 1) {
                                diagnostics.Error("E_SIR_BR_ARITY", "br requires exactly one target label", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else if (!block_labels.contains(instr.operands[0])) {
                                diagnostics.Error("E_SIR_CFG_TARGET", "br target label does not exist", diag_file,
                                                  current_range);
                                report.ok = false;
                            }
                            break;
                        case Opcode::Phi:
                            if (instr.operands.size() < 4 || (instr.operands.size() % 2) != 0) {
                                diagnostics.Error("E_SIR_PHI_ARITY", "phi requires value/label pairs", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else {
                                for (std::size_t phi_i = 0; phi_i + 1 < instr.operands.size(); phi_i += 2) {
                                    ...
                                    if (!block_labels.contains(pred_label)) {
                                        diagnostics.Error("E_SIR_PHI_LABEL", "phi predecessor label does not exist",
                                                          diag_file, current_range);
                                        report.ok = false;
                                    }
                                    if (value_type.has_value() && !instr.type.empty() &&
                                        value_type.value() != instr.type) {
                                        if (!IsTypeCompatible(instr.type, value_type.value())) {
                                            diagnostics.Error("E_SIR_PHI_TYPE", "phi incoming value type mismatch",
                                                              diag_file, current_range);
                                            report.ok = false;
                                        }
                                    }
                        ...
                        case Opcode::Drop:
                            if (instr.operands.size() != 1) {
                                diagnostics.Error("E_SIR_DROP_ARITY", "drop requires exactly one operand", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else {
                                ...
                                if (drop_counts[dropped] > 1) {
                                    diagnostics.Error("E_SIR_DOUBLE_DROP", "value dropped more than once: " + dropped,
                                                      diag_file, current_range);
                                    report.ok = false;
                                }
                            }
                        ...
                        case Opcode::Call:
                            if (instr.operands.empty()) {
                                diagnostics.Error("E_SIR_CALL_ARITY", "call requires callee operand", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else {
                                ...
                                if (sig_it != signatures.end()) {
                                    ...
                                    if (expected != actual) {
                                        diagnostics.Error("E_SIR_CALL_ARITY", "call argument count mismatch", diag_file,
                                                          current_range);
                                        report.ok = false;
                                    } else {
                                        for (std::size_t arg_i = 0; arg_i < expected; ++arg_i) {
                                            const auto arg_type =
                                                    InferOperandType(instr.operands[arg_i + 1], value_types);
                                            if (arg_type.has_value() &&
                                                !IsTypeCompatible(sig_it->second.param_types[arg_i],
                                                                  arg_type.value())) {
                                                diagnostics.Error("E_SIR_CALL_TYPE", "call argument type mismatch",
                                                                  diag_file, current_range);
                                                report.ok = false;
                                            }
                                        }
                                    ...
                                    if (!instr.type.empty() && !sig_it->second.return_type.empty() &&
                                        ...
                                        diagnostics.Error("E_SIR_CALL_RET_TYPE",
                                                          "call result type mismatch with callee signature", diag_file,
                                                          current_range);
                                        report.ok = false;
                                    }
                                }
                            ...
                            if (!instr.result.has_value()) {
                                diagnostics.Warning("W_SIR_CALL_NO_RESULT", "call result is ignored in MVP pipeline",
                                                    diag_file, current_range);
                            }
                            break;
                        case Opcode::Alloc:
                            if (!instr.result.has_value()) {
                                diagnostics.Error("E_SIR_ALLOC_RESULT", "alloc requires SSA result", diag_file,
                                                  current_range);
                                report.ok = false;
                            }
                            if (instr.operands.size() != 1) {
                                diagnostics.Error("E_SIR_ALLOC_ARITY",
                                                  "alloc requires exactly one element type operand", diag_file,
                                                  current_range);
                                report.ok = false;
                            ...
                            if (!instr.type.empty() && !IsPointerType(instr.type)) {
                                diagnostics.Error("E_SIR_ALLOC_TYPE", "alloc result type must be pointer-like",
                                                  diag_file, current_range);
                                report.ok = false;
                            }
                            break;
                        case Opcode::Load:
                            if (!instr.result.has_value()) {
                                diagnostics.Error("E_SIR_LOAD_RESULT", "load requires SSA result", diag_file,
                                                  current_range);
                                report.ok = false;
                            }
                            if (instr.operands.size() != 1) {
                                diagnostics.Error("E_SIR_LOAD_ARITY", "load requires exactly one pointer operand",
                                                  diag_file, current_range);
                                report.ok = false;
                                break;
                            }
                            if (!LooksLikeValue(instr.operands[0])) {
                                diagnostics.Error("E_SIR_LOAD_PTR", "load operand must be pointer SSA value", diag_file,
                                                  current_range);
                                report.ok = false;
                                break;
                            }
                            if (pointer_element_types.contains(instr.operands[0]) && !instr.type.empty() &&
                                ...
                                diagnostics.Error("E_SIR_LOAD_TYPE",
                                                  "load result type mismatch with pointer element type", diag_file,
                                                  current_range);
                                report.ok = false;
                            }
                            break;
                        case Opcode::Store:
                            if (instr.operands.size() != 2) {
                                diagnostics.Error("E_SIR_STORE_ARITY", "store requires value and pointer operands",
                                                  diag_file, current_range);
                                report.ok = false;
                                break;
                            }
                            if (!LooksLikeValue(instr.operands[1])) {
                                diagnostics.Error("E_SIR_STORE_PTR", "store pointer operand must be SSA value",
                                                  diag_file, current_range);
                                report.ok = false;
                                break;
                            }
                            if (pointer_element_types.contains(instr.operands[1])) {
                                ...
                                if (stored_type.has_value() &&
                                    ...
                                    diagnostics.Error("E_SIR_STORE_TYPE",
                                                      "store value type mismatch with pointer element type", diag_file,
                                                      current_range);
                                    report.ok = false;
                                }
                            }
                        ...
                        case Opcode::Ret:
                            if (instr.operands.size() != 1) {
                                diagnostics.Error("E_SIR_RET_ARITY", "ret requires exactly one operand", diag_file,
                                                  current_range);
                                report.ok = false;
                            } else {
                                const auto ret_type = InferOperandType(instr.operands[0], value_types);
                                if (ret_type.has_value() && !function.return_type.empty() &&
                                    ret_type.value() != function.return_type) {
                                    if (!IsTypeCompatible(function.return_type, ret_type.value())) {
                                        diagnostics.Error("E_SIR_RET_TYPE",
                                                          "ret operand type does not match function return type",
                                                          diag_file, current_range);
                                        report.ok = false;
                                    }
                                }
                ...
                if (terminator_count != 1) {
                    diagnostics.Error("E_SIR_TERM_COUNT", "BasicBlock must have exactly one terminator", diag_file,
                                      block_diag_range);
                    report.ok = false;
                }
            }
