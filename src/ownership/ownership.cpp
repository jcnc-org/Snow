    ...
    switch (stmt.kind) {
      ...
      case snow::frontend::Statement::Kind::Let: {
        ...
        const std::string type_name =
            !stmt.type_name.empty() ? stmt.type_name : InferExprType(stmt.expr, symbols, function_return_types);
...
OwnershipFacts OwnershipChecker::Check(const snow::sema::SemaModule& sema_module,
  ...
  for (const auto& function : sema_module.ast.functions) {
    ...
    std::stable_sort(fn_facts.begin(), fn_facts.end(),
                     [](const OwnershipFact& a, const OwnershipFact& b) { return a.declaration_index < b.declaration_index; });
...
std::string DumpOwnership(const OwnershipFacts& facts) {
  ...
  for (const auto& fact : facts.facts) {
    oss << "  - " << fact.symbol << " type=" << fact.type_name << " copy=" << (fact.is_copy_type ? "true" : "false")
