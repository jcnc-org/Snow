...
namespace {
    ...
    bool TestPassManagerConstantFold() {
        ...
        const auto result = pass_manager.Run(module, snow::passes::OptLevel::O2, snow::sir::ValidationLevel::Debug,
    ...
    bool TestLoweringUnsupportedOpcodeFails() {
        ...
        const auto emit = lowering.EmitObject(module, target, snow::passes::OptLevel::O0,
                                              "tests_unsupported_opcode_should_fail.o");
        if (emit.success) {
    ...
    bool TestParserExpressionPrecedence() {
        ...
        if (return_expr->kind != snow::frontend::Expr::Kind::Binary ||
            return_expr->op != snow::frontend::BinaryOp::Add) {
    ...
    bool TestParserWhileBreakFlag() {
        ...
        if (statements.front().body.empty() ||
            statements.front().body.front().kind != snow::frontend::Statement::Kind::Break) {
    ...
    bool TestOwnershipUseAfterMove() {
        ...
        const snow::common::SourceFile source{
                ...
                "private fn consume(a: Box) -> i32 { return 0; } pub fn main(x: Box) -> i32 { consume(x); consume(x); "
                "return 0; }",
        };
